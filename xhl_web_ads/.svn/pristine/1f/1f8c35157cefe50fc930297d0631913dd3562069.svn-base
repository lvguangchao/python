...
var play_account=null
var play_pack_id=null
...
ywl.on_init = function (cb_stack, cb_args) {
    ...
    if (show_date){
        document.getElementById("begin_time").value = "" + getFormatDate(show_date) + "";
    }else{
    ...
    $("[ywl-filter='export']").click(function () {
        ...
        play_pack_id = play_pack_id ? play_pack_id : "";
        var parm = {
            begin_time: begin_time, room_id: room_id, user_id: user_id, verify_status: play_verify_status,
            status: play_status, verify_result: play_verify_result, play_id: play_id,package_id:play_pack_id
        };
        var option = {
...
ywl.create_table_filter_verifys_list = function (tbl, selector, on_created) {
	...
	_tblf_st.filter_default = '';
	_tblf_st.filter_value = '';
	if(show_type=='2'||show_type=='1'){
        _tblf_st.filter_default = '1';
    }
	...
	_tblf_st.init = function (cb_stack) {
		...
		$(_tblf_st.selector + ' ul').empty().append($(node));
        if(show_type=='2'||show_type=='1'){
            $(_tblf_st.selector + ' button span:first').html('已审核');
        }else {
            $(_tblf_st.selector + ' button span:first').html('全部');
...
ywl.create_table_filter_verifyr_list = function (tbl, selector, on_created) {
	...
	_tblf_st.init = function (cb_stack) {
		...
		$(_tblf_st.selector + ' ul').empty().append($(node));
		if(show_type=='2'){
            $(_tblf_st.selector + ' button span:first').html('成功');
