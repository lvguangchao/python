...
def exportPlayRecord(data, list_name, filename, playnum):
    ...
    for row in range(1, len(data) + 1):
        ...
        sheet.write(row, int(0), u'%s' %
                    data[row -
                         1][0].isoformat().replace("T", " ") if data[row -
                                                                     1][0] is not None else '')
        ...
        sheet.write(row, int(8), u'%s' %
                    data[row -
                         1][6].isoformat().replace("T", " ") if data[row -
                                                                     1][6] is not None else '')
        ...
        sheet.write(row, int(13), u'%s' %
                    str(data[row -
                             1][10].quantize(Decimal('0.00'))) if data[row -
                                                                       1][10] is not None else "")
        sheet.write(row, int(14), u'%s' % data[row - 1][11])
        sheet.write(row, int(15), u'%s' %
                    str(data[row -
                             1][12].quantize(Decimal('0.00'))) if data[row -
                                                                       1][12] is not None else "")
        sheet.write(row, int(16), u'%s' % data[row - 1][13])
        sheet.write(row, int(17), u'%s' % data[row - 1][14])
        sheet.write(row, int(18), u'%s' %
                    data[row - 1][15] if data[row - 1][15] is not None else '')
        screen_shot_path = ''
        if data[row - 1][16] is not None and data[row - 1][16] != '':
            ...
            for i in arr:
                ...
                screen_shot_path = filepath.replace(
                    'pic.xiaohulu.com', 'pic.hub520.com')
                break
        ...
        # 旧视频，旧截图
        sheet.write(row, int(23), u'%s' %
                    data[row - 1][20] if data[row - 1][20] else '')
        ...
        if data[row - 1][21] is not None and data[row - 1][21] != '':
            ...
            for i in arr:
                (filepath, tempfilename) = os.path.split(i)
                screen_shot_path_old = filepath.replace(
                    'pic.xiaohulu.com', 'pic.hub520.com')
                break
...
def export_task_list_update(data, list, lits2):
    date_now = datetime.now()
    date_now = date_now.strftime('%d%H%M%S')
    datas = data[0]
    ...
    for row in range(1, len(data) + 1):
        sheet.write(row, int(0), u'%s' % data[row - 1]['task_id'])
        sheet.write(row, int(1), u'%s' % data[row - 1]['user_id'])
        sheet.write(row, int(2), u'%s' % data[row - 1]['anchor_level'])
        sheet.write(row, int(3), u'%s' % data[row - 1]['schedule_id'])
        sheet.write(row, int(4), u'%s' % data[row - 1]['plat_id'])
        sheet.write(row, int(5), u'%s' % data[row - 1]['platfrom_name'])
        sheet.write(row, int(6), u'%s' % data[row - 1]['emcee'])
        sheet.write(row, int(7), u'%s' % data[row - 1]['room_id'])
        sheet.write(row, int(8), u'%s' % data[row - 1]['task_name'])
        sheet.write(row, int(9), u'%s' % data[row - 1]['play_log'])
        sheet.write(row, int(10), u'%s' % data[row - 1]['estmate_income'])
        sheet.write(row, int(11), u'%s' % data[row - 1]['task_create_time'])
        sheet.write(row, int(12), u'%s' % data[row - 1]['stream_start_time'])
        sheet.write(row, int(13), u'%s' % data[row - 1]['estmate_income'])
        sheet.write(row, int(14), u'%s' % data[row - 1]['max_user_pre_count'])
    ...
    xls_url = os.path.join(
        ...
        str(date_now) +
        '.xls')
    excel_url = '/static/download/' + '任务页面导出_' + str(date_now) + '.xls'
    workbook.save(xls_url)
    return excel_url
...
def exportAnchor(data, list, filename):
    ...
    for row in range(1, len(data) + 1):
        ...
        sheet.write(row, int(5), u'%s' % data[row - 1]['comment'])
        sheet.write(row, int(6), u'%s' % data[row - 1]['price'])
        sheet.write(row, int(7), u'%s' % data[row - 1]['rate'])
