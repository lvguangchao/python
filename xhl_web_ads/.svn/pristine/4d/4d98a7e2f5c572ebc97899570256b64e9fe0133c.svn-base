...
ywl.on_init = function (cb_stack, cb_args) {
    ...
    var host_table_options = {
        ...
        columns: [
            ...
            {title: "广告间隔 ", key: "ads_interval",width: 10},
    ...
    $.get("/adsinfo/select", function (ret) {
            ...
            $('#ads_id5').append(auxArr.join(''));
            $('#ads_id6').append(auxArr.join(''));
            $('#ads_id7').append(auxArr.join(''));
            $('#ads_id8').append(auxArr.join(''));
        }
    );
    ...
    $("#btn-batchadd-host").click(function () {
        window.open("/needinfo/toadd")
    })
...
ywl.create_needinfo_infodlg = function (tbl) {
    ...
    needinfo_infodlg.init_dlg = function () {
        ...
        if(needinfo_infodlg.ads_id){
            ...
            for(var i in arr){
                var index =parseInt(i)+1
                $("#ads_id"+index).val(arr[i]);
                $(":checkbox[name='position'][value='" + index + "']").prop("checked", "checked");
            }
        }
