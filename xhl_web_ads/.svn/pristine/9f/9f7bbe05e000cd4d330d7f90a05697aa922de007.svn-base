...
ywl.on_init = function (cb_stack, cb_args) {
    ...
    var host_table_options = {
        ...
        columns: [
            ...
            {title: "task_id", key: "task_id",width: 50},
            ...
            {title: "套餐名", key: "package_name",width: 100},
            ...
            {title: "是否结算", key: "is_count_money",width: 50,render: "formatcount",
                fields: {is_count_money:"is_count_money"}},
            {title: "结算状态", key: "close_account",width: 50, render:'formatAccount',fields:{close_account:'close_account'}},
            ...
            // },
            {
                ...
                fields: {close_account:'close_account',is_count_money:'is_count_money',pstatus:'pstatus'}
            }
    ...
    $("[ywl-filter='export']").click(function () {
        ...
        var parm = {
            ...
            status: play_status, verify_result: play_verify_result, play_id: play_id,task_id:task_id
        }
        var option = {
            url_create: "/playrecord/export/create",
            url_select: '/playrecord/export/select',
            dom_id: '#playlog-export-info',
    ...
    ywl.create_table_filter_search_box(host_table, dom_id + " [ywl-filter='play_id']","","play_id");
    ...
    ywl.create_table_filter_package_list(host_table, dom_id + " [ywl-filter='package_type']");
    ywl.create_table_filter_account_list(host_table, dom_id + " [ywl-filter='is_account']");
...
ywl.create_playrecord_dlg = function (tbl) {
    ...
    playrecord_dlg.post =function () {
        ...
        } else  if (playrecord_dlg.update ==3||playrecord_dlg.update ==2) {
            //申诉金额
            ywl.ajax_post_json('/accountbalance/income/add', {
                    income: playrecord_dlg.income,
                    play_id: playrecord_dlg.play_id,
                    update_type: playrecord_dlg.update
                },
                function (ret) {
                    if (ret.code === TPE_OK) {
                        playrecord_dlg.tbl.reload();
...
ywl.on_host_table_created = function (tbl) {
    ...
    tbl.on_cell_created = function (row_id, col_key, cell_obj) {
        if (col_key === 'select_all') {
            ...
            $(cell_obj).find('[ywl-btn-edit]').click(function () {
                  v_dlg.update_show(row_data.play_id,row_data.income,2)
            });
    ...
    tbl.on_render_created = function (render) {
        ...
        render.make_action_btn = function (row_id, fields) {
            ...
            if (fields.close_account =='1'&&fields.is_count_money=='0'&&fields.pstatus=='1') {
                ret.push('<a href="javascript:;" class="btn btn-sm btn-primary" disabled="">修改</a>&nbsp');
                ret.push('<a href="javascript:;" class="btn btn-sm btn-primary" protocol=' + fields.protocol + ' ywl-btn-appeal="' + fields.ID + '">申诉(审核失败结算后)</a>&nbsp');
            }else if (fields.close_account =='0'){
                ret.push('<a href="javascript:;" class="btn btn-sm btn-primary" protocol=' + fields.protocol + ' ywl-btn-edit="' + fields.ID + '">修改</a>&nbsp');
                ret.push('<a href="javascript:;" class="btn btn-sm btn-primary" disabled="">申诉(审核失败结算后)</a>&nbsp');
            }else{
                ret.push('<a href="javascript:;" class="btn btn-sm btn-primary" disabled="">修改</a>&nbsp');
                ret.push('<a href="javascript:;" class="btn btn-sm btn-primary" disabled="">申诉(审核失败结算后)</a>&nbsp');
            }
        ...
        render.formatShotPath = function (row_id, fields) {
         ...
         render.formatvr = function (row_id, fields) {
          ...
          render.formatpop = function (row_id, fields) {
           ...
           render.formatcount = function (row_id, fields) {
              var temp='未知'
            if(fields.is_count_money==0){
                temp='<span class="badge badge-danger mono">不结算</span>'
            }else if(fields.is_count_money==1){
                temp='<span class="badge badge-warning mono">结算</span>'
            }
            return '<label style="color: red">'+temp+'</label>'
        };
           render.formatAccount = function (row_id, fields) {
            var temp=''
            if(fields.close_account==0){
                temp='<span class="badge badge-danger mono">No</span>'

            }else if (fields.close_account==1){
                temp='<span class="badge badge-success mono">Yes</span>'
...
ywl.create_table_filter_account_list = function (tbl, selector, on_created) {
	var _tblf_st = {};
	...
	// 过滤器内容
	_tblf_st.filter_name = 'is_account';
	_tblf_st.filter_default = '';
	_tblf_st.filter_value = '';
    // if(show_type=='2'){
	...
	_tblf_st.init = function (cb_stack) {
		var node = '';
		var user_list = ywl.page_options.user_list;
		node += '<li><a href="javascript:;" ywl-account-from="">全部</a></li>';
		node += '<li role="separator" class="divider"></li>';
		node+='<li><a href="javascript:;" ywl-account-from="' + 0+ '"> 未结算 </a></li>'
		node+='<li><a href="javascript:;" ywl-account-from="' + 1+ '"> 已结算 </a></li>'
		_tblf_st.filter_value = _tblf_st.filter_default;
		$(_tblf_st.selector + ' button span:first').html(_tblf_st.filter_value);
		$(_tblf_st.selector + ' ul').empty().append($(node));
        $(_tblf_st.selector + ' button span:first').html('全部');
		...
		// 点击事件绑定
		$(_tblf_st.selector + ' ul [ywl-account-from]').click(_tblf_st._on_select);
	...
	_tblf_st._on_select = function () {
		var account = $(this).attr("ywl-account-from");
		var account_html = $(this).html();
        play_account=account
		var cb_stack = CALLBACK_STACK.create();
		cb_stack
			.add(function (cb_stack) {
				_tblf_st.filter_value = account;
				$(_tblf_st.selector + ' button span:first').html(account_html);
				cb_stack.exec();
			});
		cb_stack.exec();
	};
...
ywl.create_table_filter_package_list = function (tbl, selector, on_created) {
	...
	_tblf_st.init = function (cb_stack) {
		var node = '';
		var pack_list = ywl.page_options.packageList;
		node += '<li><a href="javascript:;" ywl-pack-from="">全部</a></li>';
		node += '<li role="separator" class="divider"></li>';
		$.each(pack_list, function (i, p) {
			node += '<li><a href="javascript:;" ywl-pack-from="' + p[0] + '">' + p[1] + '</a></li>';
		});
		_tblf_st.filter_value = _tblf_st.filter_default;
		$(_tblf_st.selector + ' button span:first').html(_tblf_st.filter_value);
		$(_tblf_st.selector + ' ul').empty().append($(node));
		...
		// 点击事件绑定
		$(_tblf_st.selector + ' ul [ywl-pack-from]').click(_tblf_st._on_select);
	...
	_tblf_st._on_select = function () {
		var pack = $(this).attr("ywl-pack-from");
		var pack_html = $(this).html();
        play_pack_id=pack
		var cb_stack = CALLBACK_STACK.create();
		cb_stack
			.add(function (cb_stack) {
				_tblf_st.filter_value = pack;
				$(_tblf_st.selector + ' button span:first').html(pack_html);
				cb_stack.exec();
			});
		cb_stack.exec();
	};
