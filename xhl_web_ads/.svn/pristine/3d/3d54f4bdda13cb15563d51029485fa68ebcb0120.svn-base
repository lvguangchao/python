...
ywl.on_init = function (cb_stack, cb_args) {
    ...
    $("#btneed_id1").click(function () {
        ...
        var need_ids = $("#need_ids").val();
        var ads_need_group_id = $("#group_id").val();
        var type = ''
        var old_need_id = ''
...
ywl.create_needgroupinfo_info_dlg = function (tbl) {
    var needgroupinfo_info_dlg = {};
    needgroupinfo_info_dlg.dom_id = "#dialog-needgroupedit-info";
    ...
    needgroupinfo_info_dlg.update_show = function (ads_need_group_id,group_name,comment,anchor_level,row_id) {
        needgroupinfo_info_dlg.update = 1;
        needgroupinfo_info_dlg.init(ads_need_group_id,group_name, comment,anchor_level,row_id);
        // $('#dlg-notice').hide();
    ...
    needgroupinfo_info_dlg.init = function (ads_need_group_id,group_name, comment,anchor_level,row_id) {
...
ywl.on_host_table_created = function (tbl) {
    ...
    tbl.on_cell_created = function (row_id, col_key, cell_obj) {
        if (col_key === 'select_all') {
            ...
            $(cell_obj).find('[ywl-btn-group]').click(function () {
                ...
                var domid="#dialog-needgroupinfo-info"
                $(domid + ' #group_name').val(row_data.group_name);
                $(domid + ' #anchor_level').val(row_data.anchor_level);
                $(domid + ' #group_id').val(row_data.ads_need_group_id);
                $(domid + ' #need_ids').val(row_data.need_id);

                get_v_select({anchor_level: row_data.anchor_level}, function () {
                    if (row_data.need_id) {
                        var arr = row_data.need_id.split(",");
                        if (arr.length >= 1) {
                            $(domid+" #need_id1").val(arr[0])
                            $(domid+" :checkbox[name='nd1']").prop("checked", true)
                            $(domid+" #need_id1").removeAttr("disabled");

                        }
                        if (arr.length >= 2) {
                            $(domid+" #need_id2").val(arr[1])
                            $(domid+" :checkbox[name='nd2']").prop("checked", true)
                            $(domid+" #need_id2").removeAttr("disabled");
                        }
                    }
                })
                $("#dlg-notice").show()
                $("#dialog-needgroupinfo-info").modal()
            ...
            $(cell_obj).find('[ywl-btn-edit]').click(function () {
                v_dlg.update_show(row_data.ads_need_group_id,row_data.group_name,
                    row_data.comment,
                    row_data.anchor_level,row_data.row_id)
    ...
    tbl.on_render_created = function (render) {
        ...
        render.make_action_btn = function (row_id, fields) {
            var ret = [];
            ret.push('<a href="javascript:;" class="btn btn-sm btn-primary" protocol=' + fields.protocol + ' ywl-btn-group="' + fields.ID + '">分组</a>&nbsp');
