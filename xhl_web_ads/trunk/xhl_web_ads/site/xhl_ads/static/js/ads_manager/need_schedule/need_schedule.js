    ...
    ywl.ajax_post_json_time_out('/whitelist/delete', {ids: ids}, 1000 * 30,
        function (ret) {
            if (ret.code === TPE_OK) {
                ...
                $("#dialog-whiteinfo-info").modal('hide');
            } else {
...
ywl.on_init = function (cb_stack, cb_args) {
    ...
    var host_table_options = {
        ...
        columns: [
            ...
            {title: "schedule_id", key: "schedule_id", width: 10},
            {
                title: "schedule名称", key: "schedule_name", width: 220, render: 'formatname',
                fields: {schedule_name: 'schedule_name'}
            },
            ...
            // },
            {
                title: "投放时间", key: "start_alloc_time", width: 20, render: 'format_time',
    ...
    $("#dialog-whitelist-info #btn-save").click(function () {
        ...
        ywl.ajax_post_json('/whitelist/add', {
                ...
                plat_id: plat_id,
                room_id: room_id,
                ads_schedule_id_list: schedule_id_param,
                comment: comment
            },
            function (ret) {
                $("#white-add-action")[0].reset();
                if (ret.code === TPE_OK) {
                    // whitelist_infodlg.tbl.reload();
                    ywl.notify_success('添加白名单成功！');
                    // $("#dlg-notice").hide();
                    $("#dialog-whitelist-info").modal('hide');
                } else {
                    ywl.notify_error('添加白名单失败：' + ret.message);
                }
            },
            function () {
                ywl.notify_error('网络故障，添加白名单失败！');
            }
        );
    });
    ...
    $.get("/needschedule/white?id=" + id, function (ret) {
        if (ret.code === 0) {
            ...
            for (var i = 0; i < ret.data.length; i++) {
                $("#whiteinfo").append("<tr><td>" + ret.data[i].plat_id + "</td><td>" + ret.data[i].room_id + "</td><td>" + ret.data[i].ads_schedule_id + "</td><td>" + ret.data[i].create_time + "</td><td>" + ret.data[i].logtime + "</td><td>" + ret.data[i].comment + "</td><td><a href='javascript:void(0)' class='btn btn-sm btn-primary' onclick='delete_white(" + ret.data[i].id + ")'>删除</a>" + "</td><tr>")
            }
...
ywl.on_host_table_created = function (tbl) {
    ...
    tbl.on_cell_created = function (row_id, col_key, cell_obj) {
        if (col_key === 'select_all') {
            ...
            $(cell_obj).find('[ywl-btn-groupone]').click(function () {
                // var domid = "#dialog-whitelist-info";
                schedule_id_param = row_data.schedule_id;
                $("#dlg-notice").show();
                $("#dialog-whitelist-info").modal();
                $("#btn-save").click(function () {
                    ...
                    ywl.ajax_post_json('/whitelist/add', {
                            plat_id: plat_id,
                            room_id: room_id,
                            ads_schedule_id_list: schedule_id,
                            comment: comment
                        },
                        function (ret) {
                            if (ret.code === TPE_OK) {
    ...
    tbl.on_render_created = function (render) {
        ...
        render.make_action_btn = function (row_id, fields) {
            var ret = [];
            ret.push('<a href="javascript:;" class="btn btn-sm btn-danger" group_id=' + fields.ads_need_group_id + ' ywl-btn-edit="' + fields.ID + '">修改</a>&nbsp');
            ret.push('<a href="javascript:;" class="btn btn-sm btn-danger" protocol=' + fields.protocol + ' ywl-btn-retract="' + fields.ID + '">撤回</a>&nbsp');
            ret.push('<a href="javascript:;" class="btn btn-sm btn-success" protocol=' + fields.protocol + ' ywl-btn-group="' + fields.ID + '">导入白名单</a>&nbsp');
            ret.push('<a href="javascript:;" class="btn btn-sm btn-success" protocol=' + fields.protocol + ' ywl-btn-groupone="' + fields.ID + '">添加白名单</a>&nbsp');
            ret.push('<a href="javascript:void(0)" class="btn btn-sm btn-success" protocol=' + fields.protocol + ' ywl-btn-white="' + fields.ID + '">查看白名单</a>');
        ...
        render.formatname = function (row_id, fields) {
            var temp = ''
            if (fields.schedule_name) {
                var start = fields.schedule_name.indexOf('【')
                var end = fields.schedule_name.indexOf('】')
                var flag = fields.schedule_name.substr(start, end)
                temp = fields.schedule_name.replace(flag, '<span style="color: red">' + flag + '</span>');
            }
